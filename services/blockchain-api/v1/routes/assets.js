...
async function listAssets(req, res) {
  ...
  try {
    ...
    /*
    const [mainnetAssets, polygonAssets] = await Promise.all([
      (async () => {
...
async function mintAssets(
  ...
  if (MINTING_FEE > 0) {
    ...
    if (allowance.lt(fullAmountD2.v)) {
      const result = await runCustodialTransaction(mnemonic)(
        "Currency",
        "approve",
...
async function readAsset(req, res) {
  ...
  let o = await getRedisItem(assetId, redisPrefixes.polygonAsset);
  let asset = o.Item;
  if (DEVELOPMENT) setCorsHeaders(res);
  if (asset) {
...
async function deleteAsset(req, res) {
  try {
    ...
    const currentHash = await contracts["polygon"].Inventory.methods
      .getHash(assetId)
      .call();
    const randomHash = Math.random().toString(36);

    await runCustodialTransaction(MAINNET_MNEMONIC)(
      "Inventory",
      "updateHash",
      currentHash,
      randomHash
    );
    const result = await runCustodialTransaction(MAINNET_MNEMONIC)(
      "Inventory",
      "transferFrom",
      address,
      burnAddress,
  ...
  try {
    ...
    for (let i = 0; i < quantity; i++) {
      try {
        ...
        if (!isApproved) {
          await runCustodialTransaction(mnemonic)(
            "Inventory",
            "setApprovalForAll",
